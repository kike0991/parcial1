...
def seleccion_y_reproduccion(poblacion):
    ...
    poblacion_evaluada = [(indiv, calcular_fitness(indiv)) for indiv in
                          poblacion]  # Calcula el fitness de cada individuo, y lo guarda en pares ordenados de la forma (5 , [1,2,1,1,4,1,8,9,4,1])
    poblacion_ordenada = [indiv[0] for indiv in sorted(poblacion_evaluada,
                                                       key=ordenar_por_segundo_elemento)]  # Ordena los pares ordenados y se queda solo con el array de valores
    ...
    poblacion_seleccionada = poblacion_ordenada[
                             precision:]  # Esta linea selecciona los 'n' individuos del final, donde n viene dado por 'precision'
    ...
    for i in range(precision, len(poblacion)):
        ...
        poblacion[i][:punto_cruce] = padre[0][
                                     :punto_cruce]  # Se mezcla el material genetico de los padres en cada nuevo individuo
...
for ind in poblacion_ordenada:
 ...
 if mejor_fitness == 0:
     ...
     print(
         f"La solucion a la ecuacion 2X1 + 5X2² - X3 + X4 - 8X5 - 10 = 0 es {2 * individuo[0]} + {5 * individuo[1] ** 2} - {individuo[2]} + {individuo[3]} - {8 * individuo[4]} -10 = 0")
